# ICD — Протокол обмена ESP32↔UNO (I²C) — v0.3 (ПОЛНЫЙ, актуализировано под одометрию, фиксированный стеллаж и обновлённое шасси)
...
**Новое в v0.3 (по сравнению с v0.2):**
- Шасси управляется двумя ведущими каналами (левый/правый борт): `DRIVE` теперь использует `vx` и `ω`, поле `vy` зарезервировано и должно быть `0`.
- Телеметрия `DRIVEFB` сокращена до двух значений (`left_us`, `right_us`); оставшиеся байты зарезервированы.
- `SENS` содержит сырую позицию энкодера захвата вместо потенциометра; `err_flags` пополнились битом `GRIP_ENC`.
- Конфигурация захвата (`CFG_GRIP`) описывает энкодер: `enc_zero`, `enc_per_deg`, `deg_min/deg_max`.
...
**Команды (ESP32→UNO):**
- `DRIVE   0x00..0x07 (8)` — целевая (Vx, Vy(reserved), ω, t_ms)
...
- `DRIVEFB 0x50..0x57 (8)` — обратная связь PWM (левый/правый, 2×) + резерв
- `AUXFB   0x58..0x5B (4)` — PWM лифта/захвата
- `SENS    0x5C..0x5F (4)` — сырые сенсоры (энкодер захвата / лифт)
...
```
i16 vx_mm_s   | i16 vy_mm_s(res) | i16 w_mrad_s | u16 t_ms
LE            | LE               | LE           | LE
```
- `vx_mm_s` — продольная скорость (+вперёд).
- `vy_mm_s(res)` — зарезервировано, должно быть `0`; UNO игнорирует (оставлено для обратной совместимости протокола).
- `w_mrad_s` — угловая скорость (+по часовой).
...
- Ограничения скоростей и преобразование → PWM двух бортов выполняются на UNO.
...
- `err_flags` (битовая маска): `0x0001 TIMEOUT`, `0x0002 I2C`, `0x0004 ESTOP`, `0x0008 LIFT_LIMIT`, `0x0010 LIFT_ENC`, `0x0020 GRIP_ENC`, `0x0040 LINE_FAIL`, `0x0080 ODO_FAIL`, ... (расширяемо).
...
```
u16 left_us | u16 right_us | u16 res1 | u16 res2
```
- Фактические PWM (µs) на ведущие борта; `res1/res2` зарезервированы (заполнять `0`).
...
```
i16 grip_enc_cnt | i16 lift_enc_cnt
```
- Сырые счётчики энкодеров захвата и лифта (подписанные). Направление и нулевые смещения задаются конфигурацией.
...
```
i16 enc_zero | u16 enc_per_deg_q12 | i16 deg_min | i16 deg_max
```
- `enc_zero` — значение энкодера на механическом упоре (нулевая поза).
- `enc_per_deg_q12` — масштаб (count/deg) в формате фиксированной точки (Q12); UNO делит на 4096.
- Диапазон `deg_min/deg_max` определяет рабочие пределы для команд `GRIP`.
