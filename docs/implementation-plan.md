...
### 21. I2C Reliability Hotfix (2025-10-31)

- [x] Зафиксировать расхождение: в основной прошивке ESP32 UNO не виден по I2C, а в минимальном скетче обнаруживается; собрать логи `[I2C]`.
- [x] Подготовить корректировку мастер-частоты I2C (первичная 100 кГц, fallback 50 кГц) и убедиться, что параметры можно расширять дальше без конфликтов (CLI `I2C FREQ`).
- [x] После правок перепрошить `env:esp32s3`, подтвердить восстановление UNO (`[I2C] ping ok`, доступность команд) и зафиксировать вывод в журнале.
- [x] Обновить документацию/план с указанием новой частоты и влияния на другие задачи, при необходимости дополнить `CHANGELOG.md`.
- [x] Перенести шину I2C UNO на свободные GPIO ESP32-S3 (GPIO1 SDA, GPIO2 SCL) и описать требования к перепайке в проводке.
- [x] Обновить прошивку (константы `I2C_SDA`/`I2C_SCL`, защитные проверки) для новых пинов и убедиться в отсутствии конфликтов с камерой.
- [x] Обновить документацию (`docs/wire.md`) с новой распиновкой и подтвердить после аппаратной проверки.
- [x] Диагностировать отказ драйвера I2C на GPIO1/GPIO2 (`i2c driver not installed` в логе) — уточнить поддержку пинов на ESP32-S3-Freenove и, при необходимости, подобрать альтернативную пару.
- [x] Переназначить I2C SDA/SCL на альтернативные пины (GPIO14/GPIO21), обновить прошивку и проверить аппаратно.
