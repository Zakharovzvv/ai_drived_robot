# Электрика и проводка робота (RBM) — v0.4 (ПОЛНАЯ РЕДАКЦИЯ)

## Changelog v0.4
- Добавлены 2× **VEX 276‑2156** (одометрия) с новой распиновкой UNO (освобождён INT1).
- Зафиксировано крепление камеры **низко на раме** (вылет 40–60 мм, наклон 12–15° вниз).
- Расширены разделы по питанию/EMC/жгутам/Smoke‑test и контрольным измерениям.

---

## 1. Топология питания
```
[Аккумулятор 7.2–7.4V] → [Предохранитель 20–30 A] → [MPS/E‑STOP] → [MC29 ×6] → [Двигатели 393]
...
- 4× 393 (ход) до 4×2.5 A сред., пики 4×5–6 A; лифт/захват 2×2–4 A пики.  
...
## 4. Распиновка Arduino UNO (под одометрию)
...
| Захват                  | MC29       | D11         | Позиционный по потенциометру                 |
...
- Высота ~140–180 мм; вылет 40–60 мм; **наклон 12–15° вниз**; фокус OV2640 под 0.25–0.45 м.
- Жгут камеры — в стороне от силовых, перетяжки не допускаются.
...
5) Подключить энкодеры лифта/ODO, датчики линии, потенциометр.  
...
2) Все 6 Servo‑каналов ≈ 1500 µs; на MC29 — «ноль».  
3) Лифт: вращение вручную — счётчик энкодера меняется в обе стороны.  
4) ОDO: вращение каждого борта вручную — счётчики растут/убывают ожидаемо.  
5) Линия: АЦП на A0/A1 различает чёрное/белое ≥ 400 единиц.  
6) Потенциометр захвата изменяется 0..1023 без обрывов.
...
- **Архитектура v0.5** — роли, поведение, CV, калибровки.  
- **ICD v0.2** — таблица регистров, одометрия/конфиги, тайминги/ошибки.